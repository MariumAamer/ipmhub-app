...
// NOTE: this route (/topics/{id}/replies) returns 404 "rest_no_route" on
// this backend — confirmed via Postman 2026-07-10. The correct replies
// route hasn't been found yet; flagged separately, not part of this
// performance fix. Left the prefetch/map restructure in place so it's
// ready to work correctly the moment the route is fixed.
...
// ─── POST a reply ─────────────────────────────────────────────────────────────
// NOTE: this still uses the old "/topics/{id}/replies" pattern that we now
// know is wrong for GET — never separately confirmed for POST since testing
// it creates real data. Given the GET route turned out to be singular
// "reply" with "parent" as a query param, the write side may need the same
// pattern: POST /buddyboss/v1/reply with {parent: topicId, content} in the
// body. Flag to Robby / test in Postman before trusting this is correct.
export const postReply = async (
  ...
  const res = await fetch(`${BASE}/buddyboss/v1/topics/${topicId}/replies`, {
    method: 'POST',
    headers,
