...
// Per Robby: GET .../messages?box=inbox&type=unread&per_page=1&user_id={id}
// → X-WP-Total (or bbp-unread-messages) header carries the real unread
// thread total, not just what's on this page. Distinct from getThreadList()
// above, which returns the parsed thread list for the DM list screen itself.
export async function getUnreadMessageCount(userId: number): Promise<number> {
  const token = await getToken();
  const res = await fetch(
    `${BASE}/messages?box=inbox&type=unread&per_page=1&user_id=${userId}`,
    {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
  ...
  if (!res.ok) return 0;
  const total =
    res.headers.get('X-WP-Total') ?? res.headers.get('bbp-unread-messages');
