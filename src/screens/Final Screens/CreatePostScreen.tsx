...
const CreatePostScreen = ({navigation, route}: any) => {
  ...
  const uploadImages = async (
    token: string,
    activityId: number,
  ): Promise<{ids: number[]; failed: number}> => {
    const ids: number[] = [];
    let failed = 0;
    for (const img of images) {
      try {
        ...
        // Required to stop BuddyBoss from auto-creating a second, photo-only
        // activity for every uploaded image. Per BP_REST_Media_Endpoint::
        // create_item, when activity_id is empty the server calls
        // bp_activity_post_update() itself and posts the photo as its own
        // standalone activity. Passing the real (already-created) activity's
        // id attaches the photo to THAT activity instead.
        ...
        if (res.ok) {
          const data = await res.json();
          // Confirmed via adb logcat 2026-09-09: POST /buddyboss/v1/media
          // returns the created media object wrapped in an array —
          // [{ id, blog_id, attachment_id, ... }] — not a bare object. The
          // previous data?.id check always failed on this shape (arrays have
          // no .id), so every upload was reported as "failed" even though
          // the media was created successfully server-side every time.
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      // Create the text activity FIRST, before uploading any images. If
      // images are uploaded first with no activity_id, BuddyBoss's media
      // endpoint auto-creates its own separate "photo" activity for each
      // one (bp_activity_post_update() inside BP_REST_Media_Endpoint) —
      // that's the extra image-only post(s) showing up alongside the text
      // post. Creating the activity first gives every upload a real
      // activity_id to attach to instead.
      const createRes = await fetch(`${BASE}/buddyboss/v1/activity`, {
        method: 'POST',
        headers: {
      ...
      if (images.length > 0 && activityId) {
        const {ids: mediaIds, failed} = await uploadImages(token, activityId);
        if (mediaIds.length > 0) {
          await fetch(`${BASE}/buddyboss/v1/activity/${activityId}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
            },
            body: JSON.stringify({bp_media_ids: mediaIds}),
          });
        }
        if (failed > 0) {
          Alert.alert(
            'Some photos not attached',
            failed === images.length
              ? "Your post was published, but the photo(s) couldn't be attached."
              : `${failed} of ${images.length} photo(s) couldn't be attached.`,
          );
        }
      }
  ...
  const handleSchedule = async (date: string, time: string, meridiem: 'AM' | 'PM') => {
    ...
    try {
      ...
      // Create the scheduled activity FIRST, same reasoning as handlePost:
      // uploading images before the activity exists has no activity_id to
      // give them, so BuddyBoss's media endpoint auto-creates its own LIVE
      // standalone photo activity immediately — publishing it right away,
      // potentially well before the scheduled_date on the real text post.
      const body: any = {
        content: content.trim(),
        type: 'activity_update',
        component: 'activity',
      ...
      if (res.ok) {
        ...
        const activityId = created?.id;
        if (images.length > 0 && activityId) {
          const {ids: mediaIds} = await uploadImages(token, activityId);
          if (mediaIds.length > 0) {
            await fetch(`${BASE}/buddyboss/v1/activity/${activityId}`, {
              method: 'PUT',
              headers: {'Content-Type': 'application/json', Authorization: `Bearer ${token}`},
