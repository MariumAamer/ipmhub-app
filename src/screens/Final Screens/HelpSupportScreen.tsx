...
// ─── Contact details (Marium, 2026-09-11) ───────────────────────────────────
const SUPPORT_PHONE = '+353 1 6614677';
const SUPPORT_EMAIL = 'info@instituteprojectmanagement.com';
...
const HelpSupportScreen = ({navigation}: any) => {
  ...
  // No dedicated hero-subtitle field on HubPage yet — confirm with Robby via
  // Postman whether this should come from the API before hardcoding forever.
  const [heroSubtitle] = useState(
    'Search our help articles, or reach the student support team directly by phone or email.',
  );

  const [tabs, setTabs] = useState<HubTab[]>([]); // FAQ-type tabs only (glossary excluded)
  ...
  const [pendingFilterTabId, setPendingFilterTabId] = useState('');
  ...
  const loadInitial = async () => {
    ...
    const faqTabs = allTabs.filter(t => t.type !== 'glossary');
    setTabs(faqTabs);
    const support = faqTabs.find(t => t.id === 'support') || faqTabs[0] || null;
    if (support) {
  ...
  const openFilterModal = () => {
    setPendingFilterTabId(activeTab?.id || tabs[0]?.id || '');
  ...
  const applyFilter = async () => {
    const tab = tabs.find(t => t.id === pendingFilterTabId);
      ...
      <Modal transparent visible={filterModalVisible} animationType="slide" onRequestClose={closeFilterModal}>
        <View style={s.modalBackdrop}>
          ...
          <View style={s.modalSheet}>
            ...
            {tabs.map(tab => {
              const isSelected = pendingFilterTabId === tab.id;
              return (
                <TouchableOpacity
                  key={tab.id}
                  style={[s.filterOption, isSelected && s.filterOptionSelected]}
                  onPress={() => setPendingFilterTabId(tab.id)}
                  activeOpacity={0.7}>
                  <Text style={[s.filterOptionText, isSelected && s.filterOptionTextSelected]}>{tab.label}</Text>
                </TouchableOpacity>
              );
            })}
...
const s = StyleSheet.create({
  ...
  filterOptionText: {color: '#192546', fontFamily: 'Runda', fontSize: 14, fontWeight: '500'},
