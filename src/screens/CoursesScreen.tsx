...
import {
  ...
  SearchCourse,
  EnrolledCourse,
  ResumeBanner as ResumeBannerData,
  FilterGroup,
  SortOption,
} from '../api/coursesApi';
...
const MyCoursesTab = ({navigation}: {navigation: any}) => {
  ...
  const [banner, setBanner] = useState<ResumeBannerData | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      const [inProgressRes, completedRes, bannerRes] = await Promise.all([
        getMyCourses(userId),
        getCompletedCourses(userId),
        getResumeBanner(userId),
      ]);
      setCourses(inProgressRes.courses);
      setCompletedCourses(completedRes.courses);
  ...
  return (
    <ScrollView style={{width: '100%'}} showsVerticalScrollIndicator={false}>
      {/* Resume banner — only meaningful when there's an active in-progress
          course to resume; the no-active-course response shape from
          getResumeBanner() is unconfirmed, so we simply don't render rather
          than guess at an empty state. */}
      {banner && (
        <View style={{paddingHorizontal: 16, paddingTop: 24}}>
          <ResumeBanner
            banner={banner}
            onPressResume={() => {
              // CONFIRMED (Sep 2026): step_id/lesson_id/step_type were added
              // by Robby specifically so Resume can deep-link like
              // CourseDetailScreen's openStep() does, instead of landing on
              // the course overview.
              if (banner.step_type === 'quiz') {
                navigation?.navigate?.('Quiz', {
                  courseId: banner.course_id,
                  stepId: banner.step_id,
                  courseTitle: banner.title,
                  lessonTitle: banner.step_title,
                });
              } else {
                navigation?.navigate?.('StepContent', {
                  courseId: banner.course_id,
                  stepId: banner.step_id,
                  lessonId: banner.lesson_id,
                  stepType: banner.step_type,
                });
              }
            }}
          />
      ...
      {completedCourses.length > 0 && (
        // BUGFIX (Sep 2026): the gradient was only wrapping the "Completed"
        // header text as a small band, with plain white behind the cards
        // below it. Confirmed via screenshot comparison that the gradient
        // should wrap the ENTIRE section — header + all cards — so the
        // white cards float on top of a continuous blue-to-white backdrop
        // with visible margin around them, not sit on plain white right
        // below a thin header strip.
        <LinearGradient
          colors={['#ABE4FF', '#FFFFFF']}
          start={{x: 0, y: 0}}
          end={{x: 0, y: 1}}
          style={styles.completedSectionBg}>
          <Text style={styles.completedHeaderText}>{'Completed'}</Text>
          <View style={{paddingHorizontal: 16, paddingBottom: 24, gap: 16, width: '100%'}}>
            {completedCourses.map((item) => (
              <CompletedCourseCard
                key={item.id}
                course={item}
                onPressFallback={() => navigation?.navigate?.('CourseDetail', {courseId: item.id})}
              />
            ))}
          </View>
        </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  // "Completed" section header band, My Courses tab — Figma spec
  completedSectionBg: {
    width: '100%',
    paddingTop: 30,
    alignItems: 'center',
    // NOTE: this LinearGradient's end={y:1} maps to the FULL dynamic height
    // of this container (header + all cards), so with many completed
    // courses the blue->white fade stretches across all of them rather than
    // completing after the first card or two, as it does in the confirmed
    // single-card reference screenshot. Revisit with a fixed height if a
    // long completed list ends up looking too blue for too long.
  },
  completedHeaderText: {
    color: '#192647',
    fontFamily: 'Runda-Bold',
    fontSize: 18,
    marginBottom: 24, // preserves the spacing the old header band's own paddingBottom provided
  },
...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   [Sep 2026 pass — Resume Banner + Completed tab fully wired]:
   - Resume Banner: new ResumeBanner.tsx component added, wired via
     getResumeBanner(userId) fetched alongside in-progress/completed.
     Renders above the in-progress list, ONLY when the endpoint returns a
     real banner (no-active-course response shape is unconfirmed — treated
     as "don't render" rather than guessed).
   - Completed tab: CompletedCourseCard.tsx already wired via
     getCompletedCourses(); confirmed against multiple real completed-course
     samples (certificate_url, primary_recommended, is_access_expired, etc.)
   - STILL OPEN (not blocking, but worth a pixel check against Figma):
     the Resume Banner's percentage-label / last-visited row layout wasn't
     given exact CSS for that specific sub-row — built as a space-between
     row under the progress bar based on the visual mockup.
   - NOT YET BUILT (no UI spec provided): online-in-progress/online-completed
     (CRM Online courses) and the course-completers ("who completed this
     course") endpoint — both added to coursesApi.ts as data-layer functions
     only (getOnlineInProgressCourses/getOnlineCompletedCourses/
     getCourseCompleters), no screen consumes them yet.
